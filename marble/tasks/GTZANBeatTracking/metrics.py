...
from .madmom.beats import DBNBeatTrackingProcessor
...
class BCEBeatFMeasure(torchmetrics.Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        ...
        def get_idx(x):
            # x = x.squeeze(0)
            # indices = torch.nonzero((x == 1) , as_tuple=False)
...
class BeatFMeasure(BCEBeatFMeasure):
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        ...
        for batch in range(batch_size):
            ...
            # if get_idx(class_pred).size == 0:
            #     print(class_pred.shape)
            # f_measure = mir_eval.beat.f_measure(get_idx(target), get_idx(class_pred))
